with open('C:\BCA ATCS\Competitions\AdventOfCode\Advent-of-Code-2023\day11\input.txt', 'r') as f:
    ...
    GALAXIE_EXPANSION = 1 # How many rows/columns to add to the grid when there are no galaxies in a row/column
    ...
    for i in range(len(galaxies)):
        for j in range(i + 1, len(galaxies)):
            ...
            # Check if a value in rowsWithoutGalaxies is between the two galaxies' rows
            for k in range(min(galaxies[i][0], galaxies[j][0]) + 1, max(galaxies[i][0], galaxies[j][0])):
                if k in rowsWithoutGalaxies:
                    distance += GALAXIE_EXPANSION
            # Check if a value in columnsWithoutGalaxies is between the two galaxies' columns
            for k in range(min(galaxies[i][1], galaxies[j][1]) + 1, max(galaxies[i][1], galaxies[j][1])):
